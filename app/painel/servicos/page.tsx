...
export default function ServicosPage() {
    ...
    async function carregarDados() {
        ...
        setEstoque(await resEstoque.json());
        setLoading(false);
    }
                        ...
                        <div className="p-8 overflow-y-auto custom-scrollbar space-y-6">
                            ...
                            <div className="bg-gray-50 dark:bg-gray-800/50 p-5 rounded-3xl border dark:border-gray-700">
                                ...
                                <div className="flex gap-2 mb-4">
                                    <select className="flex-1 p-3 rounded-xl border dark:border-gray-700 bg-white dark:bg-gray-900 font-bold text-sm outline-none dark:text-white" value={prodSelecionado} onChange={e => setProdSelecionado(e.target.value)}>
                                        <option value="">Selecione um produto...</option>
                                        {estoque.map(p => <option key={p.id} value={p.id}>{p.name} ({p.unit})</option>)}
                                    </select>
