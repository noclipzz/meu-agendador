...
import { Calendar, Settings, Users, PlusCircle, X, Loader2, Home } from "lucide-react";
...
function PainelConteudo({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`min-h-screen flex flex-col md:flex-row font-sans ${theme}`}>
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto h-screen relative bg-gray-100 dark:bg-gray-900">{children}{isModalOpen && (<div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50"><div className="bg-white dark:bg-gray-900 p-8 rounded-2xl w-full max-w-md relative dark:text-white"><button onClick={() => setIsModalOpen(false)} className="absolute top-4 right-4"><X/></button><h3 className="text-xl font-bold mb-6">Agendar</h3><div className="space-y-4"><input className="w-full border dark:border-gray-700 p-3 rounded-lg bg-gray-50 dark:bg-gray-800" placeholder="Nome" value={novo.nome} onChange={e => setNovo({...novo, nome: e.target.value})}/><input className="w-full border dark:border-gray-700 p-3 rounded-lg bg-gray-50 dark:bg-gray-800" placeholder="Telefone" value={novo.phone} onChange={e => setNovo({...novo, phone: e.target.value})}/><div className="grid grid-cols-2 gap-4"><input type="date" className="w-full border dark:border-gray-700 p-3 rounded-lg bg-gray-50 dark:bg-gray-800" value={novo.date} onChange={e => setNovo({...novo, date: e.target.value})}/><input type="time" className="w-full border dark:border-gray-700 p-3 rounded-lg bg-gray-50 dark:bg-gray-800" value={novo.time} onChange={e => setNovo({...novo, time: e.target.value})}/></div><select className="w-full border dark:border-gray-700 p-3 rounded-lg bg-white dark:bg-gray-800" value={novo.serviceId} onChange={e => setNovo({...novo, serviceId: e.target.value})}><option value="">Serviço...</option>{services.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select><select className="w-full border dark:border-gray-700 p-3 rounded-lg bg-white dark:bg-gray-800" value={novo.professionalId} onChange={e => setNovo({...novo, professionalId: e.target.value})}><option value="">Profissional...</option>{profissionais.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select><button onClick={salvarAgendamento} className="w-full bg-green-600 text-white p-3 rounded font-bold">Confirmar</button></div></div></div>)}</main>
    </div>
  );
}
